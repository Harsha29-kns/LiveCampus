...
import { Mail, Lock, User, Briefcase, BookOpen, Check } from 'lucide-react';
...
const Register: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (success) {
            // --- THIS IS THE UPDATED SUCCESS MESSAGE ---
    ...
    const handleGoogleSignIn = async () => {
        ...
        if (success) {
            // Google sign-in might not need approval, but for consistency we show the same message.
            // If the logic differs, you can change this message.
            toast.success('Registration successful! Awaiting admin approval.');
            navigate('/login');
        }
    };
    ...
    const RoleCard = ({ value, label, icon, description }: { value: UserRole, label: string, icon: React.ReactNode, description: string }) => (
        <button
            ...
            className={`relative text-left p-4 border rounded-lg transition-all duration-200 w-full ${role === value ? 'border-indigo-500 ring-2 ring-indigo-200 shadow-md' : 'border-gray-300 hover:border-gray-400'}`}
        >
            {role === value && <span className="absolute top-2 right-2 p-1 bg-indigo-500 rounded-full"><Check size={12} className="text-white"/></span>}
    ...
    return (
        <div className="min-h-screen w-full lg:grid lg:grid-cols-2">
            ...
            <div className="hidden lg:flex flex-col items-center justify-center bg-slate-900 text-white p-12">
                <div className="w-full max-w-md">
                    <Link to="/" className="inline-block mb-8">
                        <img src="/LiveCampus.svg" alt="LiveCampus Logo" className="h-16 w-16" />
                    </Link>
                ...
                <div className="max-w-md w-full space-y-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                            <RoleCard value="student" label="Student" icon={<User size={20} className="text-indigo-600"/>} description="Register for events and join clubs."/>
                            <RoleCard value="club" label="Club" icon={<Briefcase size={20} className="text-indigo-600"/>} description="Organize events and manage members."/>
                            <RoleCard value="faculty" label="Faculty" icon={<BookOpen size={20} className="text-indigo-600"/>} description="Oversee clubs and approve events."/>
                        </div>
                    </div>
                    ...
                    <div className="relative"><div className="absolute inset-0 flex items-center"><div className="w-full border-t border-gray-300"/></div><div className="relative flex justify-center text-sm"><span className="px-2 bg-gray-50 text-gray-500">Or sign up with email</span></div></div>
